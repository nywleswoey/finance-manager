...
source; earlier "DD/MM/YYYY" layout B where the type alone sets the sign) and its
`f` / `isoA` / `isoB` helpers. `main()` is not run: it reads statements and writes
...
def test_f_strips_thousands_separators():
    assert pe.f("1,234.5678") == 1234.5678
    assert pe.f("0.30560") == 0.3056
...
def test_layout_a_buy_is_positive_units_with_price_and_amount():
    ...
    assert pe.parse_txns(txt) == [
        ("2024-03-12", "Investment", "Test Growth Fund", 1001.2345, 1.23, 1231.52)]
...
def test_layout_b_investment_positive_redemption_and_fee_negative():
    ...
    assert pe.parse_txns(txt) == [
        ("2023-04-27", "Investment", "Test Growth", 393.271, 131.85, 51852.9),
        ("2023-07-26", "Redemption", "Test Growth", -0.285, 145.62, 41.47),
        ("2023-07-26", "Endowus Fee", "Test Growth", -0.1, 145.62, 14.56),
        ("2023-07-28", "Subscription", "Test Growth", 1.0, 145.0, 145.0),
    ]
