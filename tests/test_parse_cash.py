...
Gates the date helper, the hand-made HSBC CSV's amount check (`parse_hsbc`), the DBS bank row assembly (`_name_from`, `_dbs_finish`,
...
def _parse(tmp_path, monkeypatch, amount, cr=""):
    ...
    monkeypatch.setattr(pc, "HSBC_CSV", str(path))
    return pc.parse_hsbc()
