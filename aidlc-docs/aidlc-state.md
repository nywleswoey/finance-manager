...
- **Current Stage**: CONSTRUCTION - Build and Test (unit: spending-visibility) — COMPLETE
...
| spending-visibility | Complete 2026-07-10 — see construction/spending-visibility/code/summary.md. Spending is now a per-user capability (the codebase's first). New `SPENDING_EMAILS` env var (fail closed when unset, even for ALLOWED_EMAILS accounts); `can_view_spending()` + `features_for()` in server/auth.py, recomputed from env per request so revocation needs no redeploy/re-login; ONE clause in the auth_gate middleware returns 403 for `/api/spending/*` — placed after the 401 check (anonymous sees 401, not 403) and before call_next (no handler, no DB query for a denied caller); `_is_spending()` matches exact-or-child so a future `/api/spending-export` isn't swallowed; `features` list on GET /api/auth/me; conditional nav item + activeSection fallback in App.jsx (auth.jsx unchanged — it already forwards new /me fields). The 403 is the control; hiding the menu is cosmetic (SECURITY-08). No email in the JS bundle. 19 new tests; suite 108 pass (was 89); web build clean; all 7 acceptance criteria verified live vs real DB with DEV_AUTH_BYPASS=false, incl. POST/DELETE writes 403 before validation. Security Baseline: SECURITY-08/15/06/03 compliant, rest N/A; no blocking findings. FOLLOW-UPS CLOSED 2026-07-10: Step 8 done (0556ab7) — _cfg now pins dev_auth_bypass so the local .env stops leaking in, and test_bypass_off_by_default asserts the CLASS default rather than the fixture-pinned instance; suite 115 pass / 0 fail. SPENDING_EMAILS created on Vercel Preview + Production by the user (both `vercel env pull` and `vercel env add` were denied to the agent); value unverified by the agent — must be a subset of ALLOWED_EMAILS. Takes effect on the NEXT deploy (Vercel bakes env at build time). REMAINING: pre-existing SECURITY-03 deviation at server/auth.py:150,155 (login events log email=%s) — flagged, out of scope, unfixed. |
