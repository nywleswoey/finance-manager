...
`tests/charts.spec.js` — the chart surfaces, now eight. Below 640 the donuts are not rendered and the
...
Two of the eight arrived with the composition chart and the spend trend, and both are the
*responsive* halves of those cards — what they say is `readings.spec.js`'s. The composition's is one
test carrying four claims only a browser can hold: the declared **480** at every width (one number,
...
that slot from `NetWorth.jsx` by taking its chart and the spend trend deliberately not among them,
because its four panels are one series each and their headers *are* its key — that **no source file
names a net-worth catalogue item code** — the New Snapshot form's headings and rows are derived from
the catalogue's `band`, and the codes are read out of the fixture so a recapture cannot leave the
gate asserting a stale list (`srs` is held out, because one word is a code, a band value *and* a
funding bucket, so the claim is about the other thirteen) — that **the composition's cumulative
edges are the summary tiles to the cent**, compared across the two committed payloads at the one
snapshot they share (the band order is what makes that identity hold, and nothing in a viewport
would look wrong if it moved), that **no spending surface mentions the `POSITIONAL_COLOURS` array**
— only
...
`tests/readings.spec.js` — what the two new charts **say**, as opposed to how they lay out, in a
...
is the same number at every width. "State it in words" is the recurring move of the map these cards
came from — the spend panel's delta caption, the composition's staleness pill and its per-band chip
deltas all exist because the pixels cannot carry the precision, so the words
