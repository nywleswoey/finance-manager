...
- **`1024` is a literal in two FILES** for the same reason: `styles.css`'s `max-width: 1023.98px` —
  written **three times** there since the bucket split's track floor (#157) joined the pin's block
  and the editors' `.contained`, each in a block of its own, because they are three claims about one
  width and a ticket that takes the pin to desktop must move one without the others — and
  `tests/viewports.js`'s `PIN_TIER_BELOW` / `PIN_TIER_EDGE`.
