...
1. **`.main` never scrolls horizontally.** Sideways scroll is confined to a container that is visibly a
   ...
   left and why: those three at 640/834/844 wait for the tablet tier's pin, and **five views now share
   one residual to the pixel** — `.grid2`'s 420px track floor against a 362px pane, which
   `Spending › Overview` joined exactly when its Top Line Items table became cards and **Net Worth joined
   exactly when the editors' floor landed**. Every remaining non-zero number below 640 now has that one
   cause, and no ticket owns it.
...
- **The 420px track floor is now the ONLY unowned entry in the horizontal ratchet below 640, and no
  ticket has it.** Below that width the floor exceeds the 362px pane outright, so `Portfolio ›
  Overview`, `Options`, `By Category`, `Spending › Overview` **and now `Net Worth`** all carry the
  *same* residual — 74 / 44 / 4 / 0 / 8 by viewport, one cause, no table involved. `Net Worth` joined
  the moment the editors' floor took its two tables off the pane, which is the strongest evidence the
  file can offer that the remaining phone overflow is one defect rather than five. The pinned column
